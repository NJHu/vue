...
function fetchPlugin(plugin_src, plugins_dir, options, projectRoot) {
    ...
    if (uri.hash) {
        ...
        if (result) {
            ...
            if (options.subdir) {
                events.emit('warn', 'support for subdirectories is deprecated and will be removed');
                if (options.fetch) {
    ...
    return Q.when().then(function () {
        ...
        if (uri.protocol && uri.protocol != 'file:' && uri.protocol[1] != ':' && !plugin_src.match(/^\w+:\\/) && !options.fetch) {
            ...
            return plugins.clonePluginGit(plugin_src, plugins_dir, options).fail(function (error) {
        ...
        return Q.when().then(function () {
            ...
            if (pinfo) {
                ...
                return Q.reject(new CordovaError('Plugin ' + plugin_src + ' not found locally. ' + 'Note, plugin registry was disabled by --noregistry flag.'));
            }
            ...
            if (result.skipCopyingPlugin) {
                ...
                P = Q.when(copyPlugin(result.pinfo, plugins_dir, options.link && result.fetchJsonSource.type == 'local', projectRoot));
            }
...
function loadLocalPlugins(searchpath, pluginInfoProvider) {
    if (localPlugins) {
        ...
        if (!underscore.isEqual(localPlugins.searchpath, searchpath)) {
            var msg = 'loadLocalPlugins called twice with different search paths.' + 'Support for this is not implemented.';
            throw new Error(msg);
        }
        return;
    }
...
function copyPlugin(pinfo, plugins_dir, link, projectRoot) {
    ...
    if (link) {
        ...
        // try to export web
       // var pkgdir = path.join(projectRoot, 'package.json');
      //  var pkg = require(pkgdir);
        var current_module = {};
        try {
          current_module = require(path.join(dest, 'package.json'));
        } catch(ew) {
          events.emit('warn', 'package.json Not Found');
        }

        var version = current_module.version || '*';
        try {
            if (pkg.dependencies) {
                if (!pkg.dependencies[pinfo.id]) {
                    pkg.dependencies[pinfo.id] = version;

                }
                else {
                    if (pkg.devDependencies) {
                        if (!pkg.devDependencies[pinfo.id]) {
                            pkg.devDependencies[pinfo.id] = version;
                        }
                    }
                }
                /*fs.writeFile(pkgdir, JSON.stringify(pkg, null, 2), function (err) {
                  if (err) return console.log(err);
                  events.emit('verbose', 'update' + pkgdir + 'info');
                });
                */
            }
        }
        catch (ew) {
            // events.emit('warn', 'package.json not support js module');
        }

        projectRoot = projectRoot || process.cwd() ;
        var webdest = path.join(projectRoot, 'node_modules', pinfo.id);
        if (!fs.existsSync(path.join(projectRoot, 'node_modules'))) {
            shell.mkdir('-p', path.join(projectRoot, 'node_modules'));
        }

        if (!fs.existsSync(webdest) && fs.existsSync(path.join(plugin_dir, 'js'))) {
            shell.cp('-rf', path.join(plugin_dir, 'js', '*'), webdest);
            //shell.cp('-f', path.join(plugin_dir, 'package.json'), webdest);
            var deps = current_module.dependencies || {};
            if(Object.keys(deps).length>0) {
              console.log('Install Dependences ...');
              return new Promise(function (resolve, reject) {
                let npm = child_process.exec('npm install', {cwd: webdest}, function (error, stdout, stderr) {
                  if (error) {
                    console.log();
